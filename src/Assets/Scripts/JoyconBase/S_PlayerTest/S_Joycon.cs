...
public class S_Joycon
{
    ...
    public S_Joycon(IntPtr handle_, bool imu, bool localize, float alpha, bool left)
    {
        handle = handle_;
        imu_enabled = imu;
    ...
    private void ExtractIMUValues(byte[] report_buf, int n = 0)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            UInt16 gyr_threhold = 45;// 再補正対策で少し大きめにしている
    ...
    {
        if (state <= S_Joycon.state_.ATTACHED) return;
